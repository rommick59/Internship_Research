"""XGBoost regression training/evaluation on the TBM normalized dataset.

This script uses the normalized dataset produced in this repo (values in [0, 1]).
It trains an XGBRegressor and evaluates it on multiple train/validation/test split
configurations.
...
Example (PowerShell):
  c:/Users/siame/Desktop/Stage/.venv/Scripts/python.exe Internship_Research/AI/xgboost_ml.py

You can customize split configurations:
  --splits "0.7,0.15,0.15;0.6,0.2,0.2;0.7,0.1,0.2"

You can customize core hyperparameters:
  --n-estimators 1000 --learning-rate 0.05 --max-depth 4 --subsample 0.8 --colsample-bytree 0.8
"""
...
try:
    ...
    # Allow running as a plain script: `python Internship_Research/AI/xgboost_ml.py`
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="XGBoost regression with multiple train/val/test splits")
    p.add_argument(
        "--data",
        type=Path,
    ...
    p.add_argument("--n-estimators", type=int, default=1000, help="Number of boosting rounds")
    p.add_argument("--learning-rate", type=float, default=0.05, help="Boosting learning rate")
    p.add_argument("--max-depth", type=int, default=4, help="Max depth of trees")
    p.add_argument("--subsample", type=float, default=0.8, help="Row subsample ratio")
    p.add_argument("--colsample-bytree", type=float, default=0.8, help="Column subsample ratio")
    p.add_argument("--reg-lambda", type=float, default=1.0, help="L2 regularization")

    p.add_argument(
        "--out-results",
        type=Path,
        default=Path("Internship_Research/AI/xgboost_results.csv"),

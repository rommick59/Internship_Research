"""AI8 — Train/eval models on fixed 80/20 split with TRAIN-only normalization.

Uses split files produced by:
- Internship_Research/AI8/build_split_and_normalize_train_only_80_20.py

Adds metric:
- VAF = 1 - Var(y - yhat) / Var(y)

Outputs per-model results CSVs in Internship_Research/AI8.
Models/hyperparameters match AI6.

Run (PowerShell):
    c:/Users/siame/Desktop/Stage/.venv/Scripts/python.exe Internship_Research/AI8/train_eval_models_fixed_split_80_20.py
"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="AI8: train/eval models on fixed 80/20 split")
    p.add_argument(
        "--train-csv",
        type=Path,
    ...
    p.add_argument("--out-dir", type=Path, default=Path("Internship_Research/AI8"))
    return p.parse_args()

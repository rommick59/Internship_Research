"""Support Vector Regression (SVR) training/evaluation on the TBM normalized dataset.

This script uses the normalized dataset produced in this repo (values in [0, 1]).
It trains an SVR model and evaluates it on multiple train/validation/test split
configurations.
...
Example (PowerShell):
  c:/Users/siame/Desktop/Stage/.venv/Scripts/python.exe Internship_Research/AI/svr_ml.py

You can customize split configurations:
  --splits "0.7,0.15,0.15;0.6,0.2,0.2;0.7,0.1,0.2"

You can customize the SVR hyperparameters:
  --kernel rbf --C 10 --epsilon 0.01 --gamma scale
"""
...
try:
    ...
    # Allow running as a plain script: `python Internship_Research/AI/svr_ml.py`
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="SVR regression with multiple train/val/test splits")
    p.add_argument(
        "--data",
        type=Path,
    ...
    p.add_argument("--kernel", type=str, default="rbf", help="SVR kernel (rbf, linear, poly, sigmoid)")
    p.add_argument("--C", type=float, default=10.0, help="Regularization parameter")
    p.add_argument("--epsilon", type=float, default=0.01, help="Epsilon-tube width")
    p.add_argument(
        "--gamma",
        type=str,
        default="scale",
        help="Kernel coefficient: scale|auto|<float> (for rbf/poly/sigmoid)",
    )

    p.add_argument(
        "--out-results",
        type=Path,
        default=Path("Internship_Research/AI/svr_results.csv"),
...
def main() -> int:
    ...
    for cfg in split_configs:
        ...
        model = SVR(kernel=args.kernel, C=float(args.C), epsilon=float(args.epsilon), gamma=gamma)

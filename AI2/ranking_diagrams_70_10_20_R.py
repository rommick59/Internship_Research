"""Ranking + diagrams (using R instead of R²) for the split 0.70/0.10/0.20.

This script:
1) Loads all model result CSVs in Internship_Research/AI
2) Filters the row for split train/val/test = 0.70/0.10/0.20
3) Builds rankings for each metric (train_*, val_*, test_*)
4) Generates diagrams (bar charts) for key TRAIN/TEST metrics and an overfitting view

Compared to the AI version, the diagrams use:
- R (Pearson correlation) instead of R²

Outputs:
- Internship_Research/AI2/rankings_70_10_20.csv
- Internship_Research/AI2/rankings_70_10_20.md
- Internship_Research/AI2/images/ranking_70_10_20_train.png
- Internship_Research/AI2/images/ranking_70_10_20_test.png
- Internship_Research/AI2/images/ranking_70_10_20_gap.png

Run (PowerShell):
  c:/Users/siame/Desktop/Stage/.venv/Scripts/python.exe Internship_Research/AI2/ranking_diagrams_70_10_20_R.py
...
def metric_direction(metric: str) -> str:
    """Return 'higher' if higher is better, else 'lower'."""
...
def main() -> int:
    p = argparse.ArgumentParser(description="Ranking + diagrams (R instead of R²) for split 0.70/0.10/0.20")
    p.add_argument(
        "--split",
        type=str,
        default="0.7,0.1,0.2",
    ...
    p.add_argument(
        "--ai-dir",
        type=Path,
        default=Path("Internship_Research/AI"),
        help="Folder containing *_results.csv",
    )
    p.add_argument(
        "--out-rankings-csv",
        type=Path,
        default=Path("Internship_Research/AI2/rankings_70_10_20.csv"),
    )
    p.add_argument(
        "--out-rankings-md",
        type=Path,
        default=Path("Internship_Research/AI2/rankings_70_10_20.md"),
    )
    p.add_argument(
        "--out-dir-images",
        type=Path,
        default=Path("Internship_Research/AI2/images"),
    )
    ...
    # TRAIN figure (R / RMSE / MAE)
    train_key = pd.DataFrame(
        {
            "train_r": metrics_table["train_r"],
    ...
    plot_barh(axes[0], train_key["train_r"], "TRAIN R (higher is better)", better="higher")
    ...
    # TEST figure (R / RMSE / MAE)
    test_key = pd.DataFrame(
        {
            "test_r": metrics_table["test_r"],
    ...
    fig, axes = plt.subplots(nrows=1, ncols=3, figsize=(16, 5), constrained_layout=True)
    plot_barh(axes[0], test_key["test_r"], "TEST R (higher is better)", better="higher")
    ...
    fig.suptitle(f"Overfitting view (TRAIN→TEST gap) — split {split.train:.2f}/{split.val:.2f}/{split.test:.2f}")
    out_gap = args.out_dir_images / "ranking_70_10_20_gap.png"
    fig.savefig(out_gap, dpi=200)
    plt.close(fig)

    # Console summary (top-3 per key metric)
    def top3(metric: str, better: str) -> pd.Series:
        s = metrics_table[metric]
        return s.sort_values(ascending=(better == "lower")).head(3)

    print(f"\nTop-3 (split {split.train}/{split.val}/{split.test})")
    print("train_r (higher):\n", top3("train_r", "higher").to_string())
    print("train_rmse (lower):\n", top3("train_rmse", "lower").to_string())
    print("train_mae (lower):\n", top3("train_mae", "lower").to_string())
    print("test_r (higher):\n", top3("test_r", "higher").to_string())
    print("test_rmse (lower):\n", top3("test_rmse", "lower").to_string())
    print("test_mae (lower):\n", top3("test_mae", "lower").to_string())

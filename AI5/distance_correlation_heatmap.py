"""Distance correlation heatmap (AI5).

Produces a heatmap similar to Spearman, but using distance correlation.
Distance correlation detects non-linear associations and is 0 iff independent
...
Output:
- Internship_Research/AI5/images/heatmap_distance_correlation.png

Run (PowerShell):
  c:/Users/siame/Desktop/Stage/.venv/Scripts/python.exe Internship_Research/AI5/distance_correlation_heatmap.py
"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Distance correlation heatmap")
    p.add_argument(
        "--csv",
        type=Path,
    ...
    p.add_argument(
        "--out",
        type=Path,
        default=Path("Internship_Research/AI5/images/heatmap_distance_correlation.png"),
        help="Output PNG",
    )
    p.add_argument(
        "--dpi",
...
def distance_correlation_1d(x: np.ndarray, y: np.ndarray) -> float:
    """Distance correlation for 1D variables.

    Returns value in [0, 1] (numerical noise can slightly exceed).
    """
    ...
    # If constant, dCor is 0 by definition (no variation).
    ...
    # Use the unbiased-like normalization based on Frobenius inner products.
    dcov2 = (a * b).mean()
    dvarx2 = (a * a).mean()
    dvary2 = (b * b).mean()
...
def plot_heatmap(mat: pd.DataFrame, out_path: Path, dpi: int) -> None:
    ...
    ax.set_title("Distance Correlation Heatmap")

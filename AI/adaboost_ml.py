"""AdaBoost regression (ensemble by boosting) on the TBM normalized dataset.

AdaBoost is a supervised Machine Learning (SML) ensemble method based on boosting.
This script trains an AdaBoostRegressor (with a decision tree base learner) and
evaluates it on multiple train/validation/test split configurations.
...
Example (PowerShell):
  c:/Users/siame/Desktop/Stage/.venv/Scripts/python.exe Internship_Research/AI/adaboost_ml.py

You can customize split configurations:
  --splits "0.7,0.15,0.15;0.6,0.2,0.2;0.7,0.1,0.2"

You can customize core hyperparameters:
  --n-estimators 500 --learning-rate 0.05 --max-depth 3
"""
...
try:
    ...
    # Allow running as a plain script: `python Internship_Research/AI/adaboost_ml.py`
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="AdaBoost regression with multiple train/val/test splits")
    p.add_argument(
        "--data",
        type=Path,
    ...
    p.add_argument("--n-estimators", type=int, default=500, help="Number of boosting stages")
    p.add_argument("--learning-rate", type=float, default=0.05, help="Boosting learning rate")
    p.add_argument("--max-depth", type=int, default=3, help="Max depth of base decision trees")
    p.add_argument("--min-samples-leaf", type=int, default=1, help="Min samples per leaf (base tree)")

    p.add_argument(
        "--out-results",
        type=Path,
        default=Path("Internship_Research/AI/adaboost_results.csv"),
...
def main() -> int:
    ...
    for cfg in split_configs:
        ...
        row: dict[str, object] = {
            ...
            "n_estimators": int(args.n_estimators),
            "learning_rate": float(args.learning_rate),
            "base_max_depth": int(args.max_depth),
            "base_min_samples_leaf": int(args.min_samples_leaf),
            "train": cfg.train,
            "val": cfg.val,
            "test": cfg.test,

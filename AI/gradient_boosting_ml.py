"""Gradient Boosting regression training/evaluation on the TBM normalized dataset.

This script uses the normalized dataset produced in this repo (values in [0, 1]).
It trains a scikit-learn GradientBoostingRegressor and evaluates it on multiple
...
Example (PowerShell):
  c:/Users/siame/Desktop/Stage/.venv/Scripts/python.exe Internship_Research/AI/gradient_boosting_ml.py

You can customize split configurations:
  --splits "0.7,0.15,0.15;0.6,0.2,0.2;0.7,0.1,0.2"

You can customize core hyperparameters:
  --n-estimators 500 --learning-rate 0.05 --max-depth 3 --subsample 0.8
"""
...
try:
    ...
    # Allow running as a plain script: `python Internship_Research/AI/gradient_boosting_ml.py`
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Gradient Boosting regression with multiple train/val/test splits")
    p.add_argument(
        "--data",
        type=Path,
    ...
    p.add_argument("--max-depth", type=int, default=3, help="Max depth of individual trees")
    p.add_argument("--subsample", type=float, default=1.0, help="Subsample ratio (stochastic GB when <1)")
    p.add_argument("--min-samples-leaf", type=int, default=1, help="Minimum samples per leaf")

    p.add_argument(
        "--out-results",
        type=Path,
        default=Path("Internship_Research/AI/gradient_boosting_results.csv"),
